...
*/
    ...
    public function frontend_enqueue_scripts() {
        $u = plugins_url('/', __FILE__) . 'assets/js';
        wp_deregister_script('wc-checkout');
        wp_register_script('wc-checkout',  $u. "/checkout-override.js",
    ...
    public function hide_shipping_when_free_is_available( $rates, $package ) {
        ...
        foreach($package['rates'] as $rate_id => $rate ){
            if($rate->get_label() === 'Aereo' || $rate->get_label() === 'Trayecto especial') {
                $disable_free_shipping = true;
            }
        }

        if($disable_free_shipping) {
            foreach($package['rates'] as $rate_id => $rate ){
    ...
    public function custom_product_list_column_content($column, $product_id){
        ...
        {
            case 'real_price' :
                echo money_format('%.2n', floatval($real_price)); // display the data
                break;
            ...
            case 'inventory':
                ...
                if ( $product->is_type( 'variable' ) ) {
                    ...
                    echo money_format('%.2n', floatval($total));
...
if (! wp_next_scheduled ( 'cancel_onhold_orders' )) {
    wp_schedule_event(time(), 'daily', 'cancel_onhold_orders');
}

add_action('cancel_onhold_orders', 'cancel_onhold_orders_func');
//add_action( 'init', 'do_this_hourly' );

function cancel_onhold_orders_func() {
